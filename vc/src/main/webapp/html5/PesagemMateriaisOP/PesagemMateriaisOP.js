angular.module('PesagemMateriaisOPApp', ['snk'])
    .controller('PesagemMateriaisOPController', ['DateUtils', '$scope', 'StringUtils', 'ServiceProxy', 'MessageUtils', 'i18n', 'ObjectUtils', 'Criteria', 'NumberUtils', 'SanPopup', 'MGEParameters', 'SkApplicationInstance', 'AngularUtil', '$q', 'DatasetEvents', 'CrudUtils',
        function (DateUtils, $scope, StringUtils, ServiceProxy, MessageUtils, i18n, ObjectUtils, Criteria, NumberUtils, SanPopup, MGEParameters, SkApplicationInstance, AngularUtil, $q, DatasetEvents, CrudUtils) {
        			...
        			var TELA_OP = "gerenciafretes.PesagemMateriaisOP";
        			...
        			function eventBtnConfirmar() {
        				...
        				buscaApontamentosBaseExclusao().then(apontamentosBase => {
        					...
        					if (_divergenciaApontamento) {
        						MessageUtils.showAlertWithConfirm(MessageUtils.TITLE_WARNING, i18n('GerenciaFretes.PesagemMateriaisOP.alertConvergenciaApontamento')).then(function () {
...
//        						MessageUtils
//        							.simpleConfirm(MessageUtils.TITLE_CONFIRMATION, i18n('GerenciaFretes.PesagemMateriaisOP.msgConfirmCriarApontamento'), false, false, { focusOnButton: 'ok' })
//        							.then(function () {
//        								confirmApontamentoComAfter();
//        							}, function (reason) {
//        								if (reason === 'no') {
...
//        							});
        				...
        				ServiceProxy
        					...
        					.then(function (result) {
        						...
        						if (!self.isTelaPesagemMateriaisOP) {
        							...
        							self.labelInfoPesoVolume = i18n('GerenciaFretes.PesagemMateriaisOP.lblInformeIdVolume');
        			...
        			function processaInstanciaAtividade(result) {
        				...
        				if (isMoreProdutos) {
        					...
        					var popupInstance = SanPopup.open({
        						...
        						templateUrl: 'html5/PesagemMateriaisOP/popup/ProdutosPopUp.tpl.html',
        			...
        			function setFieldsTela(records, index) {
        				if (records && records.length > 0) {
        					...
        					if (!self.isTelaPesagemMateriaisOP) {
        						self.labelInfoPesoVolume = i18n('GerenciaFretes.PesagemMateriaisOP.lblAguardandoIdVolume', [self.inputIdVolume])
        					}
        			...
        			function eventBtnReimprimir() {
        				...
        				var popupInstance = SanPopup.open({
        					...
        					templateUrl: 'html5/PesagemMateriaisOP/popup/ReimprimirPopUp.tpl.html',
        			...
        			function eventBtnConfiguracoes() {
        				...
        				var popupInstance = SanPopup.open({
        					...
        					templateUrl: 'html5/PesagemMateriaisOP/popup/ConfiguracoesPopUp.tpl.html',
...
//        				criteria.append("this.IDIPROC = ? AND this.IDIATV = ? AND this.CODPROD = ? AND (nullValue(this.CONTROLE, ' ' ) = ' ' OR this.CONTROLE = ?) AND this.NUAPO IS NULL ");
        				criteria.append("this.CODHIST > 0 ");
...
//        				}
        			...
        			function interceptColumnMetadata(fieldMetadata, dataset) {
        				if (dataset.getEntityName() == 'HistPesagemMateriais') {
        					if (fieldMetadata.id.indexOf("AD_") != 0) {
        						...
        						if (["CODHIST", "CODMATERIAL", "QTDREQUISITADA", "QTDPESADA", "STATUS"].indexOf(fieldMetadata.id) > -1) {
        			...
        			function onDatasetHistPesagemMateriaisAddCreated(dataset) {
        				...
        				dataset.whenMetadataLoaded().then(function () {
        					...
        					self.dsHistPesagemMateriaisAdd.addRecordsAsObjects({ 'CODHIST': 0 });
        			...
        			function buscaApontamentosBaseExclusao() {
        				...
        				return CrudUtils.find('HistPesagemMateriais', 'CODHIST,QTDREQUISITADA,QTDPESADA,TIPO', undefined, false, 'CODHIST IN (' + getRecordsAsString(copyRecordsSelected) + ')');
        			}

        			function buscaApontamentosBaseInclusao() {
        				return CrudUtils.find('HistPesagemMateriais', 'CODHIST,QTDREQUISITADA,QTDPESADA,TIPO', undefined, false, "CODHIST = " + self.CODHIST);
        			}
